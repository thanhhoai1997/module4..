        ...
        <div class="well well-small">
            <ul class="nav nav-list">
                <c:forEach items="${category}" var="item">
                    <li><a href='<c:url value="/san-pham/${item.id}"/>'><span class="icon-circle-blank"></span>${item.name}</a></li>
                    <li>
                </c:forEach>

                <li><a class="totalInCart" href="cart.html"><strong>Da Mua <span
        ...
        <div class="well np">
            <div id="myCarousel" class="carousel slide homCar">
                <div class="carousel-inner">
                    ...
                    <c:forEach items="${slides}" var="item" varStatus="index">
            ...
            <div class="well well-small">
                ...
                <div class="row-fluid">
                    <div id="newProductCar" class="carousel slide">
                        <div class="carousel-inner">
                            <c:if test="${productdto1.size() > 0}">
                                <div class="item active">
                                    <ul class="thumbnails">
                                            <c:forEach var="item" items="${productdto1}" varStatus="loop">
                                                <li class="span3">
                                                    <div class="thumbnail">
                                                        <a class="zoomTool" href="product_details.html" title="add to cart"><span
                                                                class="icon-search"></span> QUICK VIEW</a>
                                                ...
                                                <c:if test="${(loop.index + 1) %4==0||(loop.index+1) == productdto.size()}">
                                    </ul>

                                </div>
                                    <c:if test="${(loop.index+1)<productdto1.size()}">
                                <div class="item">
                                    <ul class="thumbnails">
                                        </c:if>
                                        </c:if>
            ...
            <div class="well well-small">
                <h3><a class="btn btn-mini pull-right" href="products.html" title="View more">VIew More<span
                        class="icon-plus"></span></a> San Pham Noi Bat </h3>
                <hr class="soften"/>
                <div class="row-fluid">

                    <c:if test="${productdto.size() > 0}">

                        <ul class="thumbnails">

                        <c:forEach var="item" items="${productdto}" varStatus="loop">

                            <li class="span4">
                                <div class="thumbnail">
                                    <a class="zoomTool" href="product_details.html" title="add to cart"><span
                                            class="icon-search"></span> QUICK VIEW</a>
                                    <a href="chi-tiet-san-pham/${item.id_product}"><img src="<c:url value="/assets/user/img/${item.img}"/>"
                                                                        alt=""></a>
                                    <div class="caption">
                                        <h5>${item.name}</h5>
                                        <h4>
                            ...
                            <c:if test="${(loop.index + 1) %3==0||(loop.index+1) == productdto.size()}">
                                </ul>
                                <c:if test="${(loop.index+1)<productdto.size()}">
                                    <ul class="thumbnails">
                                </c:if>
                            </c:if>
                        </c:forEach>
